...
const Navbar = () => {
    ...
    return (
        <>
            ...
            <div className='flex flex-row items-center text-xs px-5 py-3 space-x-2 bg-gray-100'>
                    ...
                    return (
                        <>
                            <div className='relative hidden md:flex items-center px-2 py-2 md:py-0 w-full justify-start md:h-full text-sm text-left md:cursor-pointer group' key={i}>
                                <div className={`flex font-bold items-center h-full justify-center ${(pathname === link ? ' text-red-600 border-b-2 border-red-600' : 'text-gray-600 hover:text-red-600 border-b-2 border-white hover:border-red-600')}`}>
                                ...
                                {
                                    links.submenu &&
                                    <div className='shadow-md rounded-md absolute top-12 left-0 hidden group-hover:md:block hover:md:block'>
                                        <div className='bg-white px-3 py-3 space-y-2 whitespace-nowrap'>
                                            {
                                                links.submenu.map((subItem, index) => {
                                                    return <div key={index}>
                                                        <NavLink className='text-xs border-b-2 h-4 border-white hover:border-red-600' to={subItem.link}>{subItem.title}</NavLink>
                                                    </div>
                                        ...
                                        <div className='group-hover:block hover:block'>
                                            {
                                                links.submenu.map((subItem, index) => {
                                                    return <div key={index}>
                                                        ...
                                                        <NavLink className='py-1 pl-7 hover:text-red-600 font-semibold' to={subItem.link}>{subItem.title}</NavLink>
                                                    </div>
